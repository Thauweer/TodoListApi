...
namespace TodoList.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost("token")]
        public IActionResult Token(LoginDto etryLogin)
        {
            var identity = GetIdentity(etryLogin);
            if (identity == null)
            {
        ...
        public IActionResult Registration(LoginDto entryLogin)
        {
            if(_context.Users.Where(user => user.Name == entryLogin.Login).Count() != 0)
            {
                return BadRequest("User already exist");             
            }

            if (entryLogin.Password.Length == 0 || entryLogin.Login.Length == 0)
            {
                return BadRequest("Login or password missed.");
            }

            _context.Users.Add(new User() { Name = entryLogin.Login, Password = HashPassword(entryLogin.Password) });
            _context.SaveChanges();

            var identity = GetIdentity(entryLogin);
        ...
        private ClaimsIdentity GetIdentity(LoginDto login)
        {
            try
            {
                var user = _context.Users.Where(u => u.Name == login.Login).First();
